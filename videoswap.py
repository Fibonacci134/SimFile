...
def _totensor(array):
    ...
    img = tensor.transpose(0, 1).transpose(0, 2).contiguous().cuda()
    return img.float().div(255).cuda()

def video_swap(video_path, id_vetor, swap_model, detect_model, save_path, temp_results_dir='./temp_results', crop_size=640, no_simswaplogo = False,use_mask =False):
